...
public class DetailRestaurantViewModel extends ViewModel {
    ...
    //UseCase
    //----------------------------------------------------
    private final GetRestaurantByIdUseCase getRestaurantByIdUseCase = new GetRestaurantByIdUseCase(authRepositoryFirebase);
    ...
    public StringBuilder getRatingRestaurantInStingBuilder(){
        ...
        for(int i=0;i<restaurantDetail.getNote()&&i<3;i++){
            noteToWrite.append("⭐");
        }
        return noteToWrite;
    }
    ...
    public String getTypeAndAddress(){
        String typeAndAddress = restaurantDetail.getTypeRestaurant()+" restaurant - "+ restaurantDetail.getAddress();
